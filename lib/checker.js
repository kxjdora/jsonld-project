...
class ShapeChecker {
  constructor (kb, shapeDoc, targetDoc, reportDoc, options) {
    ...
    if (reportDoc) {
      ...
      this.reportDoc = this.kb.sym('http://localhost:3000/t');
      this.kb.add(this.report, a, sh('ValidationReport'), this.reportDoc)
      console.log('add reportDoc');
    } else {
      console.log('ShapeChecker: NO REPORT DOCUMENT GIVEN')
    }
    console.log("ShapeChecker Contruction!")
  }
  ...
  execute (cb) {
    console.log('Find any shapes which should be applied to nodes')
    const checker = this
    const kb = this.kb
    this.conforms = true
    ...
    kb.statementsMatching(null, sh('targetClass'), null)
      ...
      if (this.reportDoc) {
        ...
        var conforms_obj=kb.any(this.report,sh('conforms'),null);
        kb.any(this.report,sh('conforms'),null).value=conforms_obj.value*1; 
      }

    var target=$rdf.sym("http://localhost:3000/t");
    $rdf.serialize(target,kb,null,'text/turtle',function (err, doc) {
      if (err != null) {
         console.log(err)
      } else { 
          cb(doc)
    ...
    // jsonld.fromRDF(g.toNT(), {}, function (err, doc) {
    //   if (err != null) {
    //      console.log(err)
    //   } else {    
    //     var context={
    ...
    //       }
    //     };
    //      jsonld.flatten(doc,context, null,function (err, result) {
    //        //var report_result=new ValidationReport(result)
    //        //console.log(report_result)
    //        //var str=JSON.stringify(report_result);
    //        //console.log(str);
    //        cb(result);
    //      })
    //   }              
  ...
  checkNodeShape (node, shape, avoidDuplicates) {
    ...
    kb.each(shape, sh('property')).forEach(function (property) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if(checker.isDate(object.value)){
          object.datatype = constraint;
        }
  ...
  // put new graph into kb
  addGraph(g){
    var kb = this.kb
    g.statementsMatching().forEach(function(s){
      kb.add(s.subject, s.predicate, s.object, s.way)
    })
    return this
  }
  ...
  isDate(str){
    ...
    if(hhmmss){
         ...
         var obj = new Date(year,month,date,hh,mm,ss); 
         return !!(obj.getFullYear()==year && obj.getMonth()==month && obj.getDate()==date 
         && obj.getHours()==hh && obj.getMinutes()==mm && obj.getSeconds()==ss); 
     }else{
